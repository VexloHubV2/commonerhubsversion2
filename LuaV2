...
vu1.Popup(v17, {
    Title = v15('VexloHub', vu7.Red, vu7.Green),
    Icon = 'zap',
...
local vu19 = vu18:CreateWindow({
    Title = v15('VexloHub', vu7.Red, vu7.Green),
    Author = 'VexloHub Scripts \u{2022} Latest Version',
    Folder = 'VexloHub',
    Icon = 'zap',
    NewElements = true,
...
local function vu40()
    ...
    if v38 then
        if v39 then
            ...
            print('[Gamepass Check] \u{2705} You own the premium gamepass!')
        else
            vu29 = 40

            print('[Gamepass Check] \u{274c} You do NOT own the premium gamepass.')
        end
    else
        vu29 = 40

        print('[Gamepass Check] \u{26a0}\u{fe0f} Failed to check ownership.')
    end
end
...
local v330 = vu19:Section({
    Title = v15('Vexlo Function', vu7.Pumpkin, vu7.Purple),
    Icon = 'flame',
    Opened = true,
})
local v331 = v330:Tab({
...
-- ==================== KILL ALL + RANGE SLIDER ====================
local killAllEnabled = false
local killAllConnection = nil
local killAuraRange = 25

v420:Toggle({
    Title = 'Kill All Players (Aura)',
    Desc = 'Automatically kill players in range',
    Default = false,
    Callback = function(state)
        killAllEnabled = state
        if state then
            if killAllConnection then killAllConnection:Disconnect() end
            killAllConnection = v3.Heartbeat:Connect(function()
                if not killAllEnabled then return end
                
                -- Auto equip knife
                local char = vu5.Character
                if char then
                    local knife = char:FindFirstChild('Knife') or vu5.Backpack:FindFirstChild('Knife')
                    if knife and knife.Parent \~= char then
                        knife.Parent = char
                    end
                end

                local myRoot = vu5.Character and vu5.Character:FindFirstChild('HumanoidRootPart')
                if not myRoot then return end

                for _, plr in ipairs(vu2:GetPlayers()) do
                    if plr \~= vu5 and plr.Character then
                        local hum = plr.Character:FindFirstChild('Humanoid')
                        local root = plr.Character:FindFirstChild('HumanoidRootPart')
                        if hum and hum.Health > 0 and root then
                            local distance = (myRoot.Position - root.Position).Magnitude
                            if distance <= killAuraRange then
                                myRoot.CFrame = CFrame.new(root.Position + (myRoot.Position - root.Position).Unit * 3, root.Position)
                                
                                local knife = vu5.Character:FindFirstChild('Knife')
            ...
            vu18:Notify({Title = 'Kill Aura', Content = 'Enabled | Range: ' .. killAuraRange, Icon = 'skull', Duration = 4})
        else
            if killAllConnection then 
                killAllConnection:Disconnect() 
                killAllConnection = nil 
            end
            vu18:Notify({Title = 'Kill Aura', Content = 'Disabled', Icon = 'x-circle', Duration = 3})
        end
    end,
})
...
v420:Button({
    ...
    Callback = function()
        if vu305() then
            vu18:Notify({Title = 'Knife', Content = 'Equipped successfully!', Icon = 'check-circle', Duration = 2})
        else
            vu18:Notify({Title = 'Error', Content = 'No knife found!', Icon = 'x-circle', Duration = 2})
        end
    end,
})
local vu423 = nil
...
v424:Toggle({
    ...
    Desc = 'Smooth camera lock onto murderer',
    Default = false,
    Callback = function(state)
        aimlockEnabled = state
        if state then
            if aimlockConnection then aimlockConnection:Disconnect() end
            aimlockConnection = v3.RenderStepped:Connect(function()
                local murderer = nil
                for _, plr in ipairs(vu2:GetPlayers()) do
                    if plr \~= vu5 and plr.Character then
                ...
                if murderer and murderer.Character and murderer.Character:FindFirstChild('HumanoidRootPart') then
                    ...
                    local predicted = target.Position + vel * 0.085
                    _.CFrame = CFrame.lookAt(_.CFrame.Position, predicted)
                end
            end)
            vu18:Notify({Title = 'Aimlock', Content = 'Enabled - Locked onto murderer', Icon = 'crosshair', Duration = 3})
        else
            if aimlockConnection then 
                aimlockConnection:Disconnect() 
                aimlockConnection = nil 
            end
            vu18:Notify({Title = 'Aimlock', Content = 'Disabled', Icon = 'x-circle', Duration = 2})
        end
    end,
})
...
-- Your original Shoot Murderer button (unchanged)
v424:Toggle({
    Title = 'Shoot Murderer',
    Icon = 'crosshair',
    Color = vu7.Toxic,
...
v439:Space()
v439:Section({
    Title = 'Player ESP \u{2022} Auto Farm \u{2022} Character Mods \u{2022} Teleportation \u{2022} Weapon Spawner \u{2022} Weapon Duplication \u{2022} Visual Weapons \u{2022} Trade Scam \u{2022} Role Functions \u{2022} Anti-AFK \u{2022} Server Utilities',
    TextSize = 15,
...
v439:Button({
    ...
    Callback = function()
        ...
        vu18:Notify({
            Title = 'More Scripts',
            Content = 'ADM - SAB - ETF Scripts link copied!',
            Icon = 'check-circle',
            Duration = 3,
        })
    end,
...
v439:Button({
    ...
    Justify = 'Center',
    Callback = function()
        setclipboard('https://www.youtube.com/@VexloHub')
        vu18:Notify({
            Title = 'YouTube',
...
v439:Button({
    ...
    Justify = 'Center',
    Callback = function()
        setclipboard('https://tiktok.com/@_samnycutie')
        vu18:Notify({
            Title = 'TikTok',
...
vu18:Notify({
    Title = v15('VexloHub Loaded!', vu7.Pumpkin, vu7.Purple),
    Content = 'New Best VexloHub Script Press G to toggle GUI.',
    Icon = 'skull',
    Duration = 5,
})
task.spawn(function()
